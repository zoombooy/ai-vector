...
const FunctionList: React.FC = () => {
  ...
  const loadFunctions = async () => {
    setLoading(true);
    try {
      const data = await request.get('/function/page', {
        params: {
          current: currentPage,
          size: pageSize,
  ...
  const handleDelete = async (id: number) => {
    try {
      await request.delete(`/function/${id}`);
  ...
  const handleToggleStatus = async (id: number, currentStatus: number) => {
    ...
    try {
      await request.put(`/function/${id}/status?status=${newStatus}`);
  ...
  const executeTest = async () => {
    ...
    try {
      ...
      const data = await request.post(`/function/${testFunctionId}/test`, params);
